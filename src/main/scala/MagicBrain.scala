...
class MagicBrain extends Brains[ClaimedEdges] {
  ...
  def getTargetSites(state: ClaimedEdges) : List[Site] = {
    ...
    state.targetSites.filter(site => state.our_graph.find(site) == None && graph.find(site) != None)
  }
  ...
  def getStartingPoint(state : ClaimedEdges) : Site = {
    ...
    val graph = state.graph
    debug("getStartingPoint: choosing from targets")
    ...
    debug("getStartingPoint: choosing last from history")
    ...
    debug("getStartingPoint: giving up, picking anything")
  ...
  override def nextMove(state: ClaimedEdges) : River = {
    ...
    debug(s"nextMove: ${state.graph.edges.size} rivers, ${targets.size} targets left, path: $path")
